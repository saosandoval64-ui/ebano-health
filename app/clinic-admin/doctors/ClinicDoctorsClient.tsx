...
interface ClinicDoctorsClientProps {
  initialDoctors: DoctorData[]
}

export default function ClinicDoctorsClient({ initialDoctors }: ClinicDoctorsClientProps) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      const result = editMode
        ? await updateClinicDoctor(formData)
        : await createClinicDoctor(formData)
  ...
  const handleDelete = (userId: string, docName: string) => {
    ...
    setMessage("")
    startTransition(async () => {
      const result = await deleteClinicDoctor(userId)
      setMessage(result.message)
      if (result.success) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-serif font-black tracking-tight">Médicos de la Clínica</h2>
        <button
        ...
        >
          <Plus className="h-4 w-4" /> Agregar Médico
        </button>
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-black/10 rounded-[32px] w-full max-w-lg p-6 sm:p-8 relative shadow-2xl overflow-y-auto max-h-[90vh] text-black">
            ...
            <h3 className="text-xl font-serif font-black mb-6">
              {editMode ? `Editar Médico` : `Agregar Nuevo Médico`}
            </h3>
      ...
      {doctors.length === 0 ? (
        <div className="p-12 border border-dashed border-gray-200 rounded-2xl text-center bg-gray-50">
          <p className="text-sm font-medium text-black/40">No hay médicos en tu clínica aún.</p>
        </div>
      ) : (
